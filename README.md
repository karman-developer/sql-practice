...
✅【中級SQLスキル定義】
| スキル領域                             | できるようになること                              | 業務での活用例                |
| --------------------------------- | --------------------------------------- | ---------------------- |
| ① 基本的な集計                          | `GROUP BY`, `HAVING`, `COUNT`, `SUM` など | 月別売上、部品数、カテゴリ別件数など     |
| ② サブクエリ（WHERE句 / FROM句 / SELECT句） | 平均より高いデータの抽出、ランキング計算など                  | 「平均より高い単価の部品一覧」など      |
| ③ 相関サブクエリ                         | 自分以外と比較するような条件処理                        | 「自分より高い単価が存在する部品」など    |
| ④ 複数テーブルJOIN                      | INNER JOIN, LEFT JOIN, 複数テーブルの結合        | 「製品ごとの部品一覧」「部品コスト合計」など |
| ⑤ CASE文・条件付き集計                    | 動的な条件分岐処理                               | 「在庫が0なら欠品と表示」など        |
| ⑥ サブクエリ+集計のネスト                    | サブクエリで加工 → 外側で分析                        | 「部品コストが高いカテゴリだけ抽出」など   |
